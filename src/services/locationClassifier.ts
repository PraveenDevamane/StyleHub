...
/**
 * Client-Side ML Inventory Location Suggestion Model
 * Evaluates overlapping tokens, category matching, and keyword weight mappings to output suggestions.
 */
export function classifyStorageLocation(
  ...
  const suggestions: LocationSuggestion[] = LOCATION_RULES.map(rule => {
    let score = 0;
    let matches: string[] = [];
  ...
  // If no scores are positive, return standard default fallback locations (e.g., General Rack, Main Table, Storage)
